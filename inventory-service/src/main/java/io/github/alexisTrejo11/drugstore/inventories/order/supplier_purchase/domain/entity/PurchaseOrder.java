...
import io.github.alexisTrejo11.drugstore.inventories.order.supplier_purchase.domain.entity.valueobject.*;
...
public class PurchaseOrder extends BaseDomainEntity<PurchaseOrderId> {
  ...
  private PurchaseOrder(PurchaseOrderId id, LocalDateTime createdAt, LocalDateTime updatedAt, LocalDateTime deletedAt,
      Integer version) {
  ...
  public static PurchaseOrder reconstruct(ReconstructPurchaseOrderParams params) {
    ...
    PurchaseOrder purchaseOrder = new PurchaseOrder(params.id(), params.createdAt(), params.updatedAt(),
        params.deletedAt(), params.version());
  ...
  public void validateHardDelete() {
    if (this.status != OrderStatus.CANCELLED && this.status != OrderStatus.REJECTED) {
      throw new OrderStatusValidationException(
          "Can only hard validateHardDelete orders that are cancelled or rejected");
    }
  }
  ...
  public void validateUpdate() {
    ...
    if (this.status != OrderStatus.DRAFT && this.status != OrderStatus.PENDING_APPROVAL) {
      throw new OrderStatusValidationException(
          "Only PurchaseOrders in DRAFT or PENDING_APPROVAL status can be updated.");
    }
