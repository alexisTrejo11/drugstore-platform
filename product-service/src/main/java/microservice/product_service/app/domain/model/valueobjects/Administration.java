	...
	public static Administration create(String value) {
		...
		if (!VALID_ADMINISTRATION_TYPES.contains(trimmed) &&
			...
			if (!trimmed.matches("^[A-Z\\s\\-]+$")) {
				throw new microservice.product_service.app.domain.exception.ProductValueObjectException("Administration",
						"Administration method contains invalid characters. Only letters, spaces, and hyphens are allowed.");
			}

			if (trimmed.length() < 2) {
				throw new microservice.product_service.app.domain.exception.ProductValueObjectException("Administration", "Administration method must be at least 2 characters long");
			}
		}
