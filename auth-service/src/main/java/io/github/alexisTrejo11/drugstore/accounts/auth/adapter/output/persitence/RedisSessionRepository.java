...
public class RedisSessionRepository implements SessionRepository {
  ...
  /**
   * Blacklists a specific refresh token permanently to prevent its reuse.
   * Blacklisted tokens never expire and remain blacklisted forever.
   * 
  ...
  public void blacklistRefreshToken(String refreshToken) {
    ...
    try {
      ...
      // Store permanently (no TTL) - blacklisted forever
      redisTemplate.opsForValue().set(blacklistKey, true);

      log.info("Refresh token blacklisted permanently: {}",
        ...
        if (tokenObj instanceof String token) {
          try {
            ...
            if (session != null && !session.isExpired()) {
              // Blacklist permanently (no duration needed)
