...
public interface CartJpaRepository extends JpaRepository<CartModel, String>, JpaSpecificationExecutor<CartModel> {
	...
	@Query("SELECT c FROM CartModel c LEFT JOIN FETCH c.cartItems WHERE c.id = :cartId")
	Optional<CartModel> findByIdWithCartItems(@Param("cartId") String cartId);

	@Query("SELECT c FROM CartModel c LEFT JOIN FETCH c.afterwardItems WHERE c.id = :cartId")
	...
	@Query("SELECT DISTINCT c FROM CartModel c " +
			...
			"LEFT JOIN FETCH c.afterwardItems " +
			"WHERE c.id = :cartId")
	Optional<CartModel> findByIdWithCartItemsAndAfterwards(@Param("cartId") String cartId);

	// Fetch cart by customerId with cartItems
	@Query("SELECT c FROM CartModel c LEFT JOIN FETCH c.cartItems WHERE c.customerId = :customerId")
	...
	@Query("SELECT c FROM CartModel c LEFT JOIN FETCH c.afterwardItems WHERE c.customerId = :customerId")
