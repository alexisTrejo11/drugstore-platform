...
public class CachingUserServiceDecorator implements UserService {
    ...
    public void deleteUser(UserID userID) {
        ...
        try {
            if (before != null) {
                ...
                Cache cache = cacheById();
                if (cache != null) cache.evict(userID.value());
            }
        } catch (Exception ex) {
    ...
    // validateUserCredentials is not part of the interface, so do not annotate with @Override
    ...
    private void putInCaches(User user) {
        ...
        try {
            ...
            if (byId != null) byId.put(user.getId().value(), user);

            if (user.getEmail() != null) {
                Cache byEmail = cacheByEmail();
                if (byEmail != null) byEmail.put(user.getEmail(), user);
            }

            if (user.getPhoneNumber() != null) {
                Cache byPhone = cacheByPhone();
                if (byPhone != null) byPhone.put(user.getPhoneNumber(), user);
            }

            log.debug("Put user into caches id={} email={} phone={}", user.getId().value(), user.getEmail(), user.getPhoneNumber());
        } catch (Exception ex) {
    ...
    private void evictCachesFor(User user) {
        ...
        try {
            Cache byId = cacheById();
            if (byId != null) byId.evict(user.getId().value());

            if (user.getEmail() != null) {
                Cache byEmail = cacheByEmail();
                if (byEmail != null) byEmail.evict(user.getEmail());
            }

            if (user.getPhoneNumber() != null) {
                Cache byPhone = cacheByPhone();
                if (byPhone != null) byPhone.evict(user.getPhoneNumber());
            }

            log.debug("Evicted user caches id={} email={} phone={}", user.getId().value(), user.getEmail(), user.getPhoneNumber());
        } catch (Exception ex) {
