...
import microservice.inventory_service.order.sales.core.domain.event.FulfillSaleOrderEvent;
import microservice.inventory_service.order.sales.core.domain.event.ReceiveSaleOrderEvent;
import microservice.inventory_service.order.sales.core.domain.event.UpdateSaleOrderStatusEvent;
...
public class SalesOrderUseCaseImpl implements SalesOrderUseCase {
    ...
    public void receiveSaleOrder(ReceiveOrderSaleCommand command) {
        ...
        log.info("New SaleOrder for CustomerId: {} saved successfully with OrderId: {}", command.customerId(), saleOrder.getId());
    }
